...
int calculate_departure_offset(double new_line[], double icl_line[], int HST_station_amount,
                               int ICL_station_amount, int turnover_time) {
  ...
  for (int k = 1; k < MAX_OFFSET_ALLOWED_MINUTES; k++) {
    ...
    double ICL_arrival_times[ICL_station_amount]; // the defined route will always have 9 stops in
                                                  // our example
